...
An automated pipeline that tracks how often your brand and competitors appear in AI-generated responses from ChatGPT and Google Gemini, across a defined set of high-intent queries.
...
Built to solve a real problem: most businesses have no affordable way to measure their LLM visibility. Commercial tools for this cost Rs 50,000 to Rs 2,00,000 per month. This pipeline does the same job for under $10 per month in API costs, using a Claude subscription you likely already have.
...
1. **Queries ChatGPT** via the OpenAI API with all your prompts and captures the full response
2. **Queries Google Gemini** via the Google Custom Search API and extracts AI Overview snippets where available
3. **Detects brands** in every response, recording which brands appear, in what order, and with what sentiment
4. **Compares against the previous run** and flags any changes: new brands appearing, your brand dropping out, position shifts
5. **Writes everything to Google Sheets** across 5 structured tabs, with the Dashboard tab formatted for C-Suite sharing
6. **Self-diagnoses and auto-corrects** at the start of every run: validates API keys, checks Sheet structure, recreates missing tabs, identifies and fixes errors automatically, and logs a plain-English run summary to the Dashboard
...
| Component | Cost |
|-----------|------|
| OpenAI API (gpt-4o, 15 prompts every 2 days) | ~$1 to $3 per month |
| Google Custom Search API | Free (100 queries per day free tier) |
| Google Sheets API | Free |
| GitHub Actions scheduling | Free (within free tier) |
| **Total** | **Under $10 per month** |

Compare to commercial alternatives: Profound AI and Peec AI charge Rs 50,000 to Rs 2,00,000 per month for equivalent LLM visibility tracking.

You can also add Perplexity AI as a third LLM to track. The API cost is slightly higher but the coverage is significantly wider.
...
|
+-- Phase 1: Pre-flight checks and self-diagnosis
...
|   +-- Identify and correct any errors from previous runs automatically
|
+-- Phase 2: Query ChatGPT
|   +-- chatgpt_client.py sends all prompts to gpt-4o, captures full responses
|
+-- Phase 3: Query Google Gemini
|   +-- google_client.py calls Custom Search API, extracts AI Overview snippets
|
+-- Phase 4: Brand Detection
|   +-- brand_detector.py scans each response for tracked brands plus any unlisted brands
|       +-- Records position of first mention per brand
|       +-- Scores brand sentiment (Positive / Neutral / Negative) via keyword check
...
|       +-- Logs what changed: new brand appeared, your brand dropped out, position shifted
|
+-- Phase 6: Write to Google Sheets
    +-- Tab 1 Raw Data: appends all rows from this run
    +-- Tab 2 Brand Visibility Summary: updates mention rate per prompt
    +-- Tab 3 Brand Leaderboard: re-ranks all brands by total mentions plus trend
    +-- Tab 4 Change Log: appends only rows where something changed
    +-- Tab 5 Dashboard: writes run summary block plus key metrics for executive view
```
...
| **Brand Visibility Summary** | Your brand mention rate per prompt across all runs and platforms |
| **Brand Leaderboard** | All tracked brands ranked by total mentions, with Up/Down/Same trend vs previous run |
| **Change Log** | Only the rows where something changed since the last run |
| **Dashboard** | Executive summary: overall mention rate, rank among competitors, top 3 rivals, run history |

The Dashboard tab is auto-formatted via the Sheets API: bold headers, colour-coded rows, frozen header rows, and a run summary block at the top showing timestamp, platforms queried, errors encountered, and any auto-fixes applied.

---

## The 15 Prompts (Customisable)

The prompts in this repo are configured for car insurance queries in India. You can replace them with any prompts relevant to your brand, category, and market. You can also increase the number of prompts beyond 15.
...
## Brands Being Tracked (Customisable)
...
Replace this list with your own brand and competitors. Any brand that appears in an AI response but is not in this list is automatically flagged as an unlisted brand and logged separately.
...
| Field | Description |
|-------|-------------|
| Run Date and Time | Timestamp of the query |
| Platform | ChatGPT or Google Gemini |
| Brand Mentioned | Yes or No |
| Brand Position | 1st mention, 2nd mention, not mentioned |
...
| Unlisted Brands | Any brand not in the tracking list |
| Change Detected | Yes or No vs the previous run |
...
Every run starts with a pre-flight check that validates API connectivity and Sheet structure before any queries are made. If a tab is missing from the Sheet, it is recreated automatically. If one API is down, that platform is skipped for that run and the rest continues. Errors are identified and corrected automatically before the report is generated. The run never crashes completely because of a single failure.

**Change detection as the core signal**
Raw mention counts are useful but change detection is what a content team actually acts on. The pipeline diffs every response against the previous run for the same prompt and platform, so stakeholders see immediately when your brand drops out of a response or a new competitor appears.
...
The pipeline scans for any brand name it does not recognise and flags it. This surfaces emerging competitors or aggregators that were not on the radar when the tracker was first built.

**Dashboard formatted for non-technical stakeholders**
The Sheets API writes formatting directly: bold headers, frozen rows, highlighted rows, and a plain-English run summary at the top of the Dashboard tab. The goal is a sheet that can be shared with a CMO or CFO without any manual cleanup.
...
| Google Gemini queries | Google Custom Search JSON API |
| Brand detection | Python (regex plus keyword dictionary) |
...
+-- sheets_client.py           # Google Sheets read/write plus formatting
+-- brand_detector.py          # Brand detection, position, sentiment logic
+-- config.py                  # All prompts and the brand tracking list
...
The pipeline is designed to self-diagnose and recover. Before every run it checks API connectivity and Sheet structure. During every run, a failed API call is logged and skipped rather than crashing the whole job. Errors are identified and corrected automatically. After every run, a plain-English summary is written to the Dashboard tab.
