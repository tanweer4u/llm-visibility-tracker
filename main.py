...
def run_tracker(test_mode: bool = False) -> bool:
    ...
    # ── Step 4: Clear stale data (first run only) + load previous data ──
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clear-data",
        action="store_true",
        help="Clear all data rows from Raw Data tab (keeps header). Use once to reset history.",
    )
    args = parser.parse_args()

    if args.clear_data:
        logger.info("── CLEAR DATA MODE ──")
        if not check_environment():
            sys.exit(1)
        auto_fixes: list[str] = []
        ok, spreadsheet = validate_sheet(auto_fixes)
        if not ok:
            sys.exit(1)
        removed = sheets_client.clear_raw_data(spreadsheet)
        logger.info("Done. %d data rows removed from Raw Data tab.", removed)
        sys.exit(0)
    elif args.self_test:
        ok = self_test()
